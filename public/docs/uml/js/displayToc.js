...
var icon = [];
var preloadIcons = [
  1, 10, 101, 102, 103, 104, 106, 107, 108, 11, 113, 116, 118, 16, 175, 176,
  177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 19, 20, 21,
  24, 25, 26, 28, 29, 31, 34, 4, 5, 52, 53, 56, 57, 6, 60, 68, 69, 7, 8, 82, 9,
  99,
];
...
function browserCheck() {
  ...
  if (browser.indexOf("msie") != -1) {
    if (
      browser.substring(
        browser.indexOf("msie") + 5,
        browser.indexOf(".", browser.indexOf("msie"))
      ) > 6
    )
      return "ie7";
    if (
...
function initLoad(src, toc, home) {
  ...
  if (qs.substring(0, qs.indexOf("=")) == "goto") {
    ...
    var home = "/docs/uml/EARoot/";
    for (var i = 0; i < gotoPage.length; i++) {
      home += "EA" + gotoPage[i];
      if (i != gotoPage.length - 1) home += "/";
      else home += fExt;
    }
  }
  ...
  content.innerHTML =
    "	<iframe src='" +
    toc +
    "' name='toc' id='tocIFrame' frameborder='0'></iframe>\n";
  ...
  if (qs.substring(0, qs.indexOf("=")) == "guid") {
    ...
    content.innerHTML +=
      "	<iframe src='" +
      home +
      "' name='cont' id='contentIFrame' frameborder='0'></iframe>";
...
function LoadGuidMap(ResponseFunc) {
  ...
  var src =
    "/docs/uml/js/data/guidmaps/" + "GuidMap" + FirstTwoHexDigits + ".xml";
...
function initPage(src) {
  ...
  for (var j = 0; j < cont.document.getElementsByTagName("div").length; j++) {
    if (
      cont.document.getElementsByTagName("div")[j].className ==
      "ObjectDetailsNotes"
    ) {
  ...
  if (curPage.substr(0, curPage.indexOf(".")) == "blank") return;
  ...
  if (toc.document.body == null) {
    ...
    toc.document.body.childNodes &&
    toc.document.body.childNodes.length == 1
  ) {
    setTimeout("initPage('" + src + "')", "1");
    return;
  } else if (
    toc.document.body.lastChild &&
    toc.document.body.lastChild.childNodes &&
    ...
    var parTree = curPage
      .substr(0, curPage.lastIndexOf("/"))
      .substr(curPage.indexOf("/") + 1)
      .replace(/EA/g, "")
      .replace(/\//g, ".");
    ...
    if (parTree) {
      ...
      for (var j = 0; parTree.length > j; j++) {
        ...
        var tmpCurToc = toc.document.getElementById("toc" + tocTmp).parentNode
          .childNodes;
        ...
        if (
          tmpCurToc.length > 3 &&
          tmpCurToc[tmpCurToc.length - 4].src
            .substring(
              tmpCurToc[tmpCurToc.length - 4].src.lastIndexOf("images")
            )
            .indexOf("plus") != -1
        ) {
    ...
    if (
      toc.document.getElementById(curPage).parentNode.parentNode.parentNode
        .parentNode &&
      toc.document.getElementById(curPage).parentNode.parentNode.parentNode
      ...
      if (
        toc.document
          .getElementById(curPage)
          .parentNode.parentNode.parentNode.parentNode.childNodes[
            toc.document.getElementById(curPage).parentNode.parentNode
    ...
    if (
      toc.document
        ...
        .parentNode.parentNode.childNodes[
          toc.document.getElementById(curPage).parentNode.parentNode.childNodes
            .length - 4
        ].src.indexOf("plus") != -1
    ) {
      toc.tocClick(
    ...
    if (tocSel)
      tocSel.parentNode.childNodes[
        tocSel.parentNode.childNodes.length - 2
    ...
    if (toc.document.getElementById(curPage)) {
      ...
      tocSel.parentNode.childNodes[
        tocSel.parentNode.childNodes.length - 2
      ].style.backgroundColor = "#B2B4BF";
      if (
        tocSel.offsetTop < toc.document.body.scrollTop ||
        tocSel.offsetTop >
          toc.document.body.offsetHeight + toc.document.body.scrollTop
      ) {
...
function initPreLoad(content) {
  var preload = document
    .getElementById("IndexHeader")
  ...
  if (browser == "ie6" || browser == "ie7")
    changeCSS(
      "#Preload",
      "height",
      top.document.body.clientHeight - 67 + "px",
      "this"
    );
  else changeCSS("#Preload", "height", top.innerHeight - 67 + "px", "this");

  for (var i = 0; i < preloadIcons.length; i++) {
    icon[preloadIcons[i]] = new Image();
    icon[preloadIcons[i]].src = "/docs/uml/images/" + preloadIcons[i] + ".png";
  }
...
function initPreLoaded() {
  ...
  document
    ...
    .parentNode.replaceChild(
      preloader.firstChild,
      document.getElementById("Preload")
    );
  preloader.parentNode.removeChild(preloader);

  if (browser != "op") setTimeout("setCSS();", "1000");
}
...
function resizeFramesOn(e) {
  ...
  tmpRF.style.left =
    document.getElementById("tocIFrame").clientWidth + 1 + "px";
  tmpRF.id = "tmpRF";
...
function resizeFramesOff(e) {
  ...
  if (document.getElementById("tmpRF") != null)
    document
      .getElementById("tmpRF")
      .parentNode.removeChild(document.getElementById("tmpRF"));

  resizeFrames.style.width = "5px";
  resizeFrames.style.left =
    document.getElementById("tocIFrame").clientWidth + 1 + "px";
}

function resizeFramesMove(e) {
  if (rFStatus != 0) {
    if (rFStatus == 2) var posX = e + 2;
    else if (browser == "ff") var posX = e.pageX;
    else var posX = event.x;
    ...
    document.getElementById("contentIFrame").style.width =
      top.document.body.clientWidth - posX - 4 + "px";
    if (document.getElementById("tmpRF") != null)
      document.getElementById("tmpRF").style.left = posX - 2 + "px";
    if (browser == "ie6")
      document.getElementById("tocIFrame").style.width = posX - 4 + "px";
    else document.getElementById("tocIFrame").style.width = posX - 3 + "px";

    if (tableSelTable != null) {
      if (
        tableSelTable.childNodes.length > 1 &&
        document.getElementById("tmpRF") != null
      ) {
        tableSelTable.style.width =
          document.body.clientWidth -
          (document.getElementById("tmpRF").offsetLeft + 6) -
          20 +
          "px";
        tableSelTable.childNodes[0].style.width =
          tableSelTable.offsetWidth - 19 + "px";
        tableSelTable.childNodes[1].style.width =
          tableSelTable.offsetWidth - 19 + "px";
      }
    }
  }
...
function resizePage() {
  ...
  if (document.getElementById("resizeFrames") == null) return;
...
function setCSS() {
  ...
  changeCSS(
    "#contentIFrame",
    "width",
    top.document.body.clientWidth - 258 + "px",
    "this"
  );
}
...
function tocBuild(data) {
  ...
  for (var i = 0; i < tocTab.length; i++) {
    ...
    if (
      tocTab[i][8].toLowerCase().indexOf("diagram") != -1 &&
      (tocTab[i][6] == "" || tocTab[i][6] == "0")
    )
      tocTab[i][8] = "0Diagram";
  ...
  if (tocLastID != 0) {
    var tocLastSrc = document.getElementById("toc" + tocLastID).parentNode
      .childNodes[
      document.getElementById("toc" + tocLastID).parentNode.childNodes.length -
        4
    ];
    tocLastSrc.src = tocLastSrc.src
      .substring(tocLastSrc.src.lastIndexOf("images"))
      .replace("01", "02");
  }
}
...
function tocBuildItem(i) {
  ...
  if (tocTab[i][0].indexOf(":") != -1)
    tocBranch = document.getElementById(
      "toc" + tocTab[i][0].substring(0, tocTab[i][0].lastIndexOf(":"))
    );
  ...
  if (tocTab[i][0] != 0 || tocTab[i][1] == 3) {
    ...
    if (
      tocTab[i][0].indexOf(".") == -1 &&
      (tocTab[i][0].indexOf(":") == -1 || tocTab[i][0].indexOf("0") == 0)
    )
      tocImage.src = "/docs/uml/images/join01.gif";
    else if (
      tocTab[i][0].indexOf(".") != -1 ||
      tocTab[i][0].indexOf(":") != -1
    ) {
      ...
      for (var m = 0; tocTmp.split(".").length > m + 1; m++) {
        if (
          document
            .getElementById(
              "toc" + tocTmp.substring(0, tocTmp.lastIndexOf("."))
            )
            .parentNode.childNodes[m].src.substring(
              document
                .getElementById(
                  "toc" + tocTmp.substring(0, tocTmp.lastIndexOf("."))
                )
                .parentNode.childNodes[m].src.lastIndexOf("images")
            )
            .indexOf("02") != -1 ||
          document
            .getElementById(
          ...
          tocImage.src = "/docs/uml/images/join04.gif";
        else tocImage.src = "/docs/uml/images/join03.gif";
        tocImage = tocBranch.appendChild(document.createElement("img"));
      }
      tocImage.src = "/docs/uml/images/join01.gif";
    }
    ...
    if (tocTab[i][1] == 3) {
      tocImage.src = tocImage.src
        .substring(tocImage.src.lastIndexOf("images"))
        .replace("01", "02");
      if (tocImage.parentNode.parentNode.childNodes.length > 1) {
        tocTmp =
          tocImage.parentNode.previousSibling.childNodes[
            tocImage.parentNode.previousSibling.childNodes.length - 4
          ];
        tocTmp.src = tocTmp.src
          .substring(tocTmp.src.lastIndexOf("images"))
          .replace("02", "01");
      }
    }
    ...
    if (tocTab[i][1] && tocTab[i][1] != 3) {
      ...
      while (
        tocImage.parentNode.childNodes.length <
        document.getElementById(tocTab[i][6]).parentNode.childNodes.length - 2
      ) {
        tocImage.parentNode.insertBefore(
          document.createElement("img"),
          tocImage
        ).src = "/docs/uml/images/join03.gif";
      }
      tocImage.src = tocImage.src
        .substring(tocImage.src.lastIndexOf("images"))
        .replace("01", "02");
      if (
        document.getElementById(tocTab[i][6]).parentNode.lastChild.childNodes
          .length > 1
      ) {
        tocTmpSrc = document.getElementById(tocTab[i][6]).parentNode.lastChild
          .lastChild.previousSibling.childNodes[
          document.getElementById(tocTab[i][6]).parentNode.lastChild.lastChild
            .previousSibling.childNodes.length - 4
        ];
        tocTmpSrc.src = tocTmpSrc.src
          .substring(tocTmpSrc.src.lastIndexOf("images"))
          .replace("02", "01");
      }
      tocImage = document.getElementById(tocTab[i][6]).parentNode.childNodes[
        document.getElementById(tocTab[i][6]).parentNode.childNodes.length - 4
      ];
  ...
  if (top.icon[tocTab[i][4]]) tocImg.src = top.icon[tocTab[i][4]].src;
  else tocImg.src = "/docs/uml/images/" + tocTab[i][4] + ".png";

  if (tocTab[i][3]) tocImg.id = tocTab[i][3];
  ...
  if (tocTab[i][3]) {
    ...
    if (tocTab[i][9]) tocText.id = tocTab[i][9];
    else if (tocTab[i][7]) tocText.id = tocTab[i][7];
...
function tocClick(tocSrc) {
  if (
    tocSrc.src.substring(tocSrc.src.lastIndexOf("images")).indexOf("minus") !=
    -1
  ) {
    tocSrc.parentNode.childNodes[
      tocSrc.parentNode.childNodes.length - 1
    ].style.display = "none";
    tocSrc.src = tocSrc.src
      .substring(tocSrc.src.lastIndexOf("images"))
      .replace("minus", "plus");
  } else if (
    tocSrc.src.substring(tocSrc.src.lastIndexOf("images")).indexOf("plus") != -1
  ) {
    tocSrc.parentNode.childNodes[
      tocSrc.parentNode.childNodes.length - 1
    ].style.display = "block";
    ...
    if (
      tocSrc.parentNode.childNodes[tocSrc.parentNode.childNodes.length - 1]
        .childNodes.length == 0
    ) {
...
function tocInit() {
  if (browser == "ie_old") {
    top.document.getElementById("Preload").childNodes[0].innerHTML =
...
function tocLoadDataProcess() {
  if (data.readyState == 4) {
    ...
    try {
      ...
      if (tocInitRoot == 2) {
        tocLoadData(
          tocTab[0][7].substring(1, tocTab[0][7].length - 1) + ".xml"
        );
      } else if (
        tocInitRoot == 3 &&
        top.document.getElementById("Preload") != null
      ) {
        tocInitRoot = 0;
        top.initPreLoaded();
      }
...
function toggleDiv(idDiv, idImage) {
  ...
  if (ele && ele.style.display == "none") {
    ...
    cont.document.getElementById(idImage).src =
      cont.document
...
function toggleData(src) {
  var dataRoot =
    cont.document.getElementById(src).parentNode.parentNode.parentNode;
  if (
    cont.document
      .getElementById(src)
      .src.substring(
    ...
    cont.document.getElementById(src).src =
      cont.document
        .getElementById(src)
        .src.substring(
          0,
  ...
  for (var i = 0; i < dataRoot.getElementsByTagName("tr").length; i++) {
    if (
      dataRoot.getElementsByTagName("tr")[i].id.indexOf(src) != -1 &&
      cont.document
        .getElementById(src)
        .src.substring(
          cont.document.getElementById(src).src.lastIndexOf("images")
        ...
        .indexOf("minus") != -1 &&
      dataRoot.getElementsByTagName("tr")[i].getElementsByTagName("td")[1]
        .firstChild != null
    )
...
function toggleItem(item, type) {
  if (tableSel != null) {
    cont.document.getElementById(tableSel + "Title").style.background =
      "#FFFFFF";
  ...
  if (browser == "ff" || browser == "op" || browser == "ie7") {
    ...
    var contWWidth =
      document.body.clientWidth -
      (top.document.getElementById("resizeFrames").offsetLeft + 6);
  }
  ...
  if (cont.document.body.offsetHeight - contWHeight > 0) {
    ...
    if (browser == "ff" || browser == "op") {
      tableSelTable.style.height =
        tableSelTable.offsetHeight -
        (cont.document.body.offsetHeight - cont.innerHeight) -
        12 +
        "px";
    } else {
      tableSelTable.style.height =
        tableSelTable.offsetHeight -
        (cont.document.body.offsetHeight - contWHeight) -
        3 +
        "px";
      if (cont.document.body.offsetWidth > contWWidth || type == null) {
        ...
        if (tableSelTable.getElementsByTagName("table").length != 0)
          tableSelTable.getElementsByTagName("table")[0].style.width =
            tableSelTable.getElementsByTagName("table")[0].offsetWidth -
            18 +
            "px";
      }
    }
    if (tableSelTable.getElementsByTagName("table").length != 0) {
      tabHead = tableSelTable.appendChild(
        tableSelTable.getElementsByTagName("table")[0].cloneNode(true)
      );
    ...
    cont.document.getElementById("TableGroup").offsetHeight +
      cont.document.getElementById("TableGroup").offsetTop <
      contWHeight - 6 &&
    tableSelTable.style.overflow == "scroll"
  ) {
    if (browser == "ff" || browser == "op") {
